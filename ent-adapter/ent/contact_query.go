...
func (_q *ContactQuery) QueryCompany() *CompanyQuery {
...
func (_q *ContactQuery) QueryOwner() *UserQuery {
...
func (_q *ContactQuery) WithCompany(opts ...func(*CompanyQuery)) *ContactQuery {
	query := (&CompanyClient{config: _q.config}).Query()
...
func (_q *ContactQuery) WithOwner(opts ...func(*UserQuery)) *ContactQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ContactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Contact, error) {
	...
	if _q.withCompany != nil || _q.withOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withCompany; query != nil {
		if err := _q.loadCompany(ctx, query, nodes, nil,
			func(n *Contact, e *Company) { n.Edges.Company = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ContactQuery) loadCompany(ctx context.Context, query *CompanyQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *Company)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_contacts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ContactQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Contact)
	for i := range nodes {
