...
func (cq *CompanyQuery) QueryContacts() *ContactQuery {
...
func (cq *CompanyQuery) WithContacts(opts ...func(*ContactQuery)) *CompanyQuery {
	query := &ContactQuery{config: cq.config}
...
func (cq *CompanyQuery) sqlAll(ctx context.Context) ([]*Company, error) {
	...
	if query := cq.withContacts; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Contacts = []*Contact{}
		}
		query.withFKs = true
		query.Where(predicate.Contact(func(s *sql.Selector) {
			s.Where(sql.InValues(company.ContactsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "company_contacts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "company_contacts" returned %v for node %v`, *fk, n.ID)
			}
